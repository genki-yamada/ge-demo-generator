...
# --- Cleanup Mode Handler ---
  if [ "$CLEANUP_MODE" = "true" ]; then
    ...
    echo "  • BigQuery Dataset: ge_demo_minimal_ds"
    echo "  • Maps API Key: MCP-Demo-Key-min01"
    echo "  • Cloud Run Main Service: ge-demo-minimal (if deployed)"
    echo "  • Cloud Run Live Viewer Function: ge-demo-minimal-viewer"
    echo "  • Firestore Collection: ge-demo-minimal-data"
    echo "  • Gemini Enterprise registration (App): ge-demo-minimal"
    echo "  • Custom MCP Secrets in Secret Manager (if exist)"
    echo "  • Agent Engine (Sandbox): ge-demo-minimal-sandbox"
    echo "  • Pub/Sub Topics: ge-demo-minimal-sched-tasks, ge-demo-minimal-task-results"
    echo "  • Pub/Sub Subscriptions: ge-demo-minimal-sched-tasks-push, ge-demo-minimal-task-results-push"
    echo "  • Cloud Scheduler Jobs: ge-demo-minimal-sched-* (if any)"
    echo "  • Firestore Task Collections: ge-demo-minimal_task_definitions, ge-demo-minimal_task_executions"
    echo "  • Local Directory: ~/ge-demo-minimal"
    echo ""
    _HAS_SLACK=$(gcloud secrets describe "ge-demo-minimal-slack-token" --project="$PROJECT_ID" 2>/dev/null && echo "yes" || echo "no")
    if [ "$_HAS_SLACK" = "yes" ]; then
      echo "⚠️  Manual cleanup required after deletion:"
      echo "  • Slack App: GE-ge-demo-minimal"
    ...
    echo ""
    echo "🗑️  Deleting BigQuery Dataset: ge_demo_minimal_ds..."
    bq rm -r -f -d $PROJECT_ID:ge_demo_minimal_ds 2>/dev/null && echo "   ✅ Dataset deleted." || echo "   ⚠️  Dataset not found or already deleted."
    
    echo ""
    echo "🔑 Deleting Maps API Key: MCP-Demo-Key-min01..."
    KEY_NAME=$(gcloud alpha services api-keys list --filter="displayName:MCP-Demo-Key-min01" --format="value(name)" 2>/dev/null || echo "")
    if [ ! -z "$KEY_NAME" ]; then
      DELETED_ALL=true
    ...
    MAIN_REGION=$(gcloud run services list --filter="metadata.name:ge-demo-minimal" --format="value(region)" 2>/dev/null | head -n 1)
    if [ ! -z "$MAIN_REGION" ]; then
      gcloud run services delete ge-demo-minimal --region="$MAIN_REGION" --quiet 2>/dev/null && echo "   ✅ Cloud Run main service deleted." || echo "   ⚠️  Failed to delete Main service."
    else
    ...
    VIEWER_REGION=$(gcloud run services list --filter="metadata.name:ge-demo-minimal-viewer" --format="value(region)" 2>/dev/null | head -n 1)
    if [ ! -z "$VIEWER_REGION" ]; then
      gcloud functions delete ge-demo-minimal-viewer --gen2 --region="$VIEWER_REGION" --quiet 2>/dev/null && echo "   ✅ Live Viewer Cloud Run Function deleted." || echo "   ⚠️  Failed to delete Live Viewer Function."
    else
    ...
    echo ""
    echo "🔥 Deleting Firestore Collection: ge-demo-minimal-data..."
    if command -v uv >/dev/null 2>&1; then
      GOOGLE_API_USE_CLIENT_CERTIFICATE=false uv run --with google-cloud-firestore python3 -c "from google.cloud import firestore; db=firestore.Client(); [d.reference.delete() for d in db.collection('ge-demo-minimal-data').stream()]" 2>/dev/null && echo "   ✅ Firestore documents in collection deleted." || echo "   ⚠️  Could not clear Firestore collection automatically."
    fi
    ...
    for LOC in "global" "us" "eu"; do
      ...
      for E_NAME in $(echo "$ENGINES_JSON" | jq -r '.engines[]? | .name'); do
        ...
        for A_NAME in $(echo "$ASSISTANTS" | jq -r '.assistants[]? | .name'); do
          ...
          TARGET_AGENT_NAME=$(echo "$AGENTS_JSON" | jq -r --arg dir "ge-demo-minimal" '.agents[]? | select(.a2aAgentDefinition.jsonAgentCard != null) | select((.a2aAgentDefinition.jsonAgentCard | fromjson | .name) == $dir) | .name' 2>/dev/null | head -n 1)
    ...
    AUTH_PATH="projects/$PROJECT_ID/locations/global/authorizations/ge-demo-minimal-auth"
    ...
    if [ "$_AUTH_EXISTS" = "200" ]; then
      echo ""
      echo "🔐 Deleting Gemini Enterprise Authorization Resource: ge-demo-minimal-auth..."
    ...
    MCP_SECRETS=$(gcloud secrets list --format="value(name)" 2>/dev/null | grep "min01" || true)
    ...
    _AE_NAME=""
    if [ -f ~/ge-demo-minimal/.env ]; then
      _AE_NAME=$(grep '^AGENT_ENGINE_NAME=' ~/ge-demo-minimal/.env | sed 's/^AGENT_ENGINE_NAME=//' | sed 's/^"//;s/"$//')
    fi
    if [ -n "$_AE_NAME" ]; then
...
" || echo "   ⚠️  Agent Engine deletion failed. You may need to delete it manually from the console."
    ...
    for SUB in "ge-demo-minimal-sched-tasks-push" "ge-demo-minimal-task-results-push"; do
    ...
    for TOP in "ge-demo-minimal-sched-tasks" "ge-demo-minimal-task-results"; do
    ...
    SCHED_JOBS=$(gcloud scheduler jobs list --location=us-central1 --project="$PROJECT_ID" \
      --format="value(name)" 2>/dev/null | grep "ge-demo-minimal-sched-" || true)
...
db = firestore.Client()
for coll_name in ['ge-demo-minimal_task_definitions', 'ge-demo-minimal_task_executions', 'ge-demo-minimal_task_push_configs']:
...
" 2>/dev/null || echo "   ⚠️  Could not clear Firestore task collections."
    ...
    if gcloud secrets describe "ge-demo-minimal-slack-token" --project="$PROJECT_ID" >/dev/null 2>&1; then
      echo ""
      echo "📱 Slack App (manual cleanup required):"
      echo "   ⚠️  Please delete the Slack App manually at: https://api.slack.com/apps"
      echo "   Look for an app named 'GE-ge-demo-minimal' and delete it."
    fi
...
while true; do
  ...
  echo '🤖 Agent Name:    MinimalAgent (ge-demo-minimal)'
  echo '📝 Description:   A minimal demo agent with no data.'
  echo "📂 Demo Asset Directory: ~/ge-demo-minimal"
...
if [ -d "ge-demo-minimal" ]; then
  echo "📂 Removing existing directory ge-demo-minimal for a clean setup..."
  rm -rf "ge-demo-minimal"
fi

# --- 3. Data Provisioning ---
echo "🗄 Creating BigQuery Dataset: ge_demo_minimal_ds..."
bq mk --dataset --location=US ge_demo_minimal_ds 2>/dev/null || echo "    ✅ Dataset already exists."
...
mkdir -p ge-demo-minimal/adk_agent/app
cd ge-demo-minimal
...
API_KEY_JSON=$(gcloud alpha services api-keys create --display-name="MCP-Demo-Key-min01"     --api-target=service=mapstools.googleapis.com     --format=json 2>/dev/null || echo "")
...
if [ ! -z "$API_KEY_JSON" ]; then
    ...
    API_KEY=$(gcloud alpha services api-keys list --filter="displayName:MCP-Demo-Key-min01" --format="value(keyString)" 2>/dev/null || echo "")
fi
...
for _attempt in range(_MAX_RETRIES):
    try:
        agent_engine = client.agent_engines.create(
            config={'display_name': 'ge-demo-minimal-sandbox'},
        )
        break
...
DEMO_DATASET="ge_demo_minimal_ds"
...
__FAST_API_EOF__
  perl -pi -e "s/tmp-ref-run/ge-demo-minimal/g" app/fast_api_app.py 2>/dev/null || true
...
# --- 9. Final Launch & Tips ---
  ...
  SERVICE_NAME="ge-demo-minimal"

  # --- Pre-create Pub/Sub topics in background (no dependency on SERVICE_URL) ---
  SCHED_TOPIC="ge-demo-minimal-sched-tasks"
  RESULT_TOPIC="ge-demo-minimal-task-results"
  ...
  CR_ENV_VARS="GOOGLE_CLOUD_PROJECT=$PROJECT_ID,GOOGLE_CLOUD_LOCATION=global,MAPS_API_KEY=$API_KEY,GEMINI_AUTHORIZATION_ID=$AUTH_ID,ADK_ENABLE_MCP_GRACEFUL_ERROR_HANDLING=1,ADK_DISABLE_JSON_SCHEMA_FOR_FUNC_DECL=1,DEMO_ID=ge-demo-minimal"
if [ "$VIEWER_DEPLOYED" = "true" ]; then
...
EOF
  ...
  if [ "$APP_COUNT" = "1" ]; then
    ...
    REG_OUTPUT=$(python3 register_agent.py "$SELECTED_LOC" "$PROJECT_NUMBER" "$SELECTED_LOC" "$SELECTED_APP_ID" "$TOKEN" "ge-demo-minimal" "$SERVICE_URL/a2a/app" 'MinimalAgent' 'A minimal demo agent with no data.' "$AUTH_ID")
    echo "$REG_OUTPUT"
  ...
  echo "📂 Demo Asset Directory: ~/ge-demo-minimal"
  echo "📊 BigQuery Dataset:    ge_demo_minimal_ds"
  echo "🔥 Firestore:           ge-demo-minimal-data"
  ...
  echo "   👉 https://console.cloud.google.com/bigquery?referrer=search&project=$PROJECT_ID&ws=!1m4!1m3!3m2!1s$PROJECT_ID!2sge_demo_minimal_ds"
  echo ""
  echo "========================================================="
  echo ""
  echo "💡 Next Steps:"
  ...
  echo "  $ cd ~ && bash setup-ge-demo-minimal.sh --cleanup"
  echo "========================================================="
  exit 0
