...
# --- Cleanup Mode Handler ---
  if [ "$CLEANUP_MODE" = "true" ]; then
    ...
    echo "  • BigQuery Dataset: ge_demo_mcp_ds"
    echo "  • Maps API Key: MCP-Demo-Key-mcp01"
    echo "  • Cloud Run Main Service: ge-demo-mcp (if deployed)"
    echo "  • Cloud Run Live Viewer Function: ge-demo-mcp-viewer"
    echo "  • Firestore Collection: ge-demo-mcp-data"
    echo "  • Gemini Enterprise registration (App): ge-demo-mcp"
    echo "  • Custom MCP Secrets in Secret Manager (if exist)"
    echo "  • Agent Engine (Sandbox): ge-demo-mcp-sandbox"
    echo "  • Pub/Sub Topics: ge-demo-mcp-sched-tasks, ge-demo-mcp-task-results"
    echo "  • Pub/Sub Subscriptions: ge-demo-mcp-sched-tasks-push, ge-demo-mcp-task-results-push"
    echo "  • Cloud Scheduler Jobs: ge-demo-mcp-sched-* (if any)"
    echo "  • Firestore Task Collections: ge-demo-mcp_task_definitions, ge-demo-mcp_task_executions"
    echo "  • Local Directory: ~/ge-demo-mcp"
    echo ""
    _HAS_SLACK=$(gcloud secrets describe "ge-demo-mcp-slack-token" --project="$PROJECT_ID" 2>/dev/null && echo "yes" || echo "no")
    if [ "$_HAS_SLACK" = "yes" ]; then
      echo "⚠️  Manual cleanup required after deletion:"
      echo "  • Slack App: GE-ge-demo-mcp"
    ...
    echo ""
    echo "🗑️  Deleting BigQuery Dataset: ge_demo_mcp_ds..."
    bq rm -r -f -d $PROJECT_ID:ge_demo_mcp_ds 2>/dev/null && echo "   ✅ Dataset deleted." || echo "   ⚠️  Dataset not found or already deleted."
    
    echo ""
    echo "🔑 Deleting Maps API Key: MCP-Demo-Key-mcp01..."
    KEY_NAME=$(gcloud alpha services api-keys list --filter="displayName:MCP-Demo-Key-mcp01" --format="value(name)" 2>/dev/null || echo "")
    if [ ! -z "$KEY_NAME" ]; then
      DELETED_ALL=true
    ...
    MAIN_REGION=$(gcloud run services list --filter="metadata.name:ge-demo-mcp" --format="value(region)" 2>/dev/null | head -n 1)
    if [ ! -z "$MAIN_REGION" ]; then
      gcloud run services delete ge-demo-mcp --region="$MAIN_REGION" --quiet 2>/dev/null && echo "   ✅ Cloud Run main service deleted." || echo "   ⚠️  Failed to delete Main service."
    else
    ...
    VIEWER_REGION=$(gcloud run services list --filter="metadata.name:ge-demo-mcp-viewer" --format="value(region)" 2>/dev/null | head -n 1)
    if [ ! -z "$VIEWER_REGION" ]; then
      gcloud functions delete ge-demo-mcp-viewer --gen2 --region="$VIEWER_REGION" --quiet 2>/dev/null && echo "   ✅ Live Viewer Cloud Run Function deleted." || echo "   ⚠️  Failed to delete Live Viewer Function."
    else
    ...
    echo ""
    echo "🔥 Deleting Firestore Collection: ge-demo-mcp-data..."
    if command -v uv >/dev/null 2>&1; then
      GOOGLE_API_USE_CLIENT_CERTIFICATE=false uv run --with google-cloud-firestore python3 -c "from google.cloud import firestore; db=firestore.Client(); [d.reference.delete() for d in db.collection('ge-demo-mcp-data').stream()]" 2>/dev/null && echo "   ✅ Firestore documents in collection deleted." || echo "   ⚠️  Could not clear Firestore collection automatically."
    fi
    ...
    for LOC in "global" "us" "eu"; do
      ...
      for E_NAME in $(echo "$ENGINES_JSON" | jq -r '.engines[]? | .name'); do
        ...
        for A_NAME in $(echo "$ASSISTANTS" | jq -r '.assistants[]? | .name'); do
          ...
          TARGET_AGENT_NAME=$(echo "$AGENTS_JSON" | jq -r --arg dir "ge-demo-mcp" '.agents[]? | select(.a2aAgentDefinition.jsonAgentCard != null) | select((.a2aAgentDefinition.jsonAgentCard | fromjson | .name) == $dir) | .name' 2>/dev/null | head -n 1)
    ...
    AUTH_PATH="projects/$PROJECT_ID/locations/global/authorizations/ge-demo-mcp-auth"
    ...
    if [ "$_AUTH_EXISTS" = "200" ]; then
      echo ""
      echo "🔐 Deleting Gemini Enterprise Authorization Resource: ge-demo-mcp-auth..."
    ...
    MCP_SECRETS=$(gcloud secrets list --format="value(name)" 2>/dev/null | grep "mcp01" || true)
    ...
    _AE_NAME=""
    if [ -f ~/ge-demo-mcp/.env ]; then
      _AE_NAME=$(grep '^AGENT_ENGINE_NAME=' ~/ge-demo-mcp/.env | sed 's/^AGENT_ENGINE_NAME=//' | sed 's/^"//;s/"$//')
    fi
    if [ -n "$_AE_NAME" ]; then
...
" || echo "   ⚠️  Agent Engine deletion failed. You may need to delete it manually from the console."
    ...
    for SUB in "ge-demo-mcp-sched-tasks-push" "ge-demo-mcp-task-results-push"; do
    ...
    for TOP in "ge-demo-mcp-sched-tasks" "ge-demo-mcp-task-results"; do
    ...
    SCHED_JOBS=$(gcloud scheduler jobs list --location=us-central1 --project="$PROJECT_ID" \
      --format="value(name)" 2>/dev/null | grep "ge-demo-mcp-sched-" || true)
...
db = firestore.Client()
for coll_name in ['ge-demo-mcp_task_definitions', 'ge-demo-mcp_task_executions', 'ge-demo-mcp_task_push_configs']:
...
" 2>/dev/null || echo "   ⚠️  Could not clear Firestore task collections."
    ...
    if gcloud secrets describe "ge-demo-mcp-slack-token" --project="$PROJECT_ID" >/dev/null 2>&1; then
      echo ""
      echo "📱 Slack App (manual cleanup required):"
      echo "   ⚠️  Please delete the Slack App manually at: https://api.slack.com/apps"
      echo "   Look for an app named 'GE-ge-demo-mcp' and delete it."
    fi
...
while true; do
  ...
  echo '🤖 Agent Name:    McpAgent (ge-demo-mcp)'
  echo '📝 Description:   An MCP-enabled agent with GitHub and Workspace integrations.'
  echo "📂 Demo Asset Directory: ~/ge-demo-mcp"
  echo "🧠 Agent Models:   root_agent: $AGENT_MODEL_LITE / deep_analysis_agent: $AGENT_MODEL"
  echo "🧪 Code Sandbox:   ✅ Enabled (Agent Runtime)"
  echo "🔌 Google Workspace MCP: Enabled"
echo "🔌 Imported MCP #1:  my-mcp-server"
echo "========================================================="
...
while true; do
  read -s -p "▶ Enter EXAMPLE_API_KEY (API key for ExampleMCP service): " EXAMPLE_API_KEY
  echo ""
  if [ -n "$EXAMPLE_API_KEY" ]; then break; fi
  echo "  ⚠️  EXAMPLE_API_KEY is required. Please enter a value."
done
read -p "▶ Enter EXAMPLE_API_URL (Base URL for ExampleMCP service) [OPTIONAL - press Enter to skip]: " EXAMPLE_API_URL
...
if [ -d "ge-demo-mcp" ]; then
  echo "📂 Removing existing directory ge-demo-mcp for a clean setup..."
  rm -rf "ge-demo-mcp"
fi

# --- 3. Data Provisioning ---
echo "🗄 Creating BigQuery Dataset: ge_demo_mcp_ds..."
bq mk --dataset --location=US ge_demo_mcp_ds 2>/dev/null || echo "    ✅ Dataset already exists."
...
cat <<'__CSV_EOF__' > tasks.csv
task_id,title,priority,status
T001,Setup Cloud,high,pending
T002,Test Agent,medium,pending
__CSV_EOF__
bq_fail=0
echo "📊 Loading tables in parallel..."
cat << 'EOF' | xargs -P 5 -n 4 ./load_table.sh
tasks tasks.csv task_id:STRING,title:STRING,priority:STRING,status:STRING ge_demo_mcp_ds
EOF
if [ $? -ne 0 ]; then
  bq_fail=1
fi
...
mkdir -p ge-demo-mcp/adk_agent/app
cd ge-demo-mcp
...
cat <<'__DOCKER_MCP_CLONE_0_EOF__' >> Dockerfile
RUN git clone https://github.com/example-org/my-mcp-server /app/custom_mcp_0
__DOCKER_MCP_CLONE_0_EOF__

cat <<'__DOCKER_MCP_INSTALL_0_EOF__' >> Dockerfile
RUN cd /app/custom_mcp_0 && (if [ -f pyproject.toml ] || [ -f setup.py ]; then uv pip install --system . 2>/dev/null || true; elif [ -f requirements.txt ]; then uv pip install --system -r requirements.txt; fi) || (npm install && npm run build 2>/dev/null || true)
__DOCKER_MCP_INSTALL_0_EOF__

cat <<'__START_SH_EOF__' > start_mcp.sh
#!/bin/bash

TOTAL_SIDECARS=1
echo "🔌 Starting $TOTAL_SIDECARS MCP sidecars in parallel..."

# Phase 1: Launch all sidecars in parallel
cd /app/custom_mcp_0 && supergateway --stdio "" --outputTransport streamableHttp --port 9090 --sessionStateless &
PID_0=$!
...
for _attempt in $(seq 1 30); do
  ALL_READY=true
  for P in $PORTS; do
    case " $READY " in *" $P "*) continue ;; esac
    if curl -s -m 2 -o /dev/null -w "" http://127.0.0.1:$P/mcp 2>/dev/null; then
      echo "  ✅ Port $P ready (${_attempt}s)"
      READY="$READY $P"
    else
      ALL_READY=false
    fi
...
# Report results
READY_COUNT=$(echo $READY | wc -w | tr -d " ")
echo "✅ $READY_COUNT/$TOTAL_SIDECARS MCP sidecars ready"
for P in $PORTS; do
  case " $READY " in *" $P "*) ;; *) echo "  ⚠️ Port $P did not become ready in time" ;; esac
done

echo "🚀 Starting ADK agent..."
cd /app
exec uvicorn adk_agent.app.fast_api_app:app --host 0.0.0.0 --port 8080
__START_SH_EOF__
...
__DOCKER_TAIL_EOF__
echo 'CMD ["/bin/bash", "/app/start_mcp.sh"]' >> Dockerfile
...
API_KEY_JSON=$(gcloud alpha services api-keys create --display-name="MCP-Demo-Key-mcp01"     --api-target=service=mapstools.googleapis.com     --format=json 2>/dev/null || echo "")
...
if [ ! -z "$API_KEY_JSON" ]; then
    ...
    API_KEY=$(gcloud alpha services api-keys list --filter="displayName:MCP-Demo-Key-mcp01" --format="value(keyString)" 2>/dev/null || echo "")
fi
...
for _attempt in range(_MAX_RETRIES):
    try:
        agent_engine = client.agent_engines.create(
            config={'display_name': 'ge-demo-mcp-sandbox'},
        )
        break
...
def get_custom_mcp_toolsets():
    ...
    toolsets = []
    mcp_configs = [
        {"idx": 0, "port": 9090, "entrypoint": "undefined", "required_keys": "EXAMPLE_API_KEY", "name": "examplemcp"},
    ]
...
4. **Custom MCP Toolset #1 (my-mcp-server)**: Access data in the custom MCP server.
...
# --- Shared tools list ---
_all_tools = [t for t in [maps_toolset, bigquery_toolset, firestore_toolset, tools.generate_image, slack_mcp_toolset] + custom_mcp_toolsets + [tools.get_gmail_mcp_toolset(), tools.get_drive_mcp_toolset(), tools.get_calendar_mcp_toolset(), tools.get_chat_mcp_toolset(), tools.get_people_mcp_toolset()] if t is not None]
...
__FAST_API_EOF__
  perl -pi -e "s/tmp-ref-run/ge-demo-mcp/g" app/fast_api_app.py 2>/dev/null || true
...
echo "🔑 Provisioning Secrets for MCP #1 (my-mcp-server)..."
if [ -z "$EXAMPLE_API_KEY" ]; then
  EXAMPLE_API_KEY="UNDEFINED"
fi
if gcloud secrets describe ge-demo-mcp-my-server-example-api-key >/dev/null 2>&1; then
  echo -n "$EXAMPLE_API_KEY" | gcloud secrets versions add ge-demo-mcp-my-server-example-api-key --data-file=-
else
  echo -n "$EXAMPLE_API_KEY" | gcloud secrets create ge-demo-mcp-my-server-example-api-key --data-file=- --replication-policy="automatic"
fi
  ...
  SERVICE_NAME="ge-demo-mcp"

  # --- Pre-create Pub/Sub topics in background (no dependency on SERVICE_URL) ---
  SCHED_TOPIC="ge-demo-mcp-sched-tasks"
  RESULT_TOPIC="ge-demo-mcp-task-results"
  ...
  CR_ENV_VARS="GOOGLE_CLOUD_PROJECT=$PROJECT_ID,GOOGLE_CLOUD_LOCATION=global,MAPS_API_KEY=$API_KEY,GEMINI_AUTHORIZATION_ID=$AUTH_ID,ADK_ENABLE_MCP_GRACEFUL_ERROR_HANDLING=1,ADK_DISABLE_JSON_SCHEMA_FOR_FUNC_DECL=1,DEMO_ID=ge-demo-mcp,EXAMPLE_API_URL=$EXAMPLE_API_URL"
if [ "$VIEWER_DEPLOYED" = "true" ]; then
...
gcloud run deploy "$SERVICE_NAME"     --source ..     --memory "8Gi"     --cpu 2     --no-cpu-throttling     --cpu-boost     --min-instances 1     --timeout 1800     --no-allow-unauthenticated     --ingress internal     --labels "created-by=adk"     --set-env-vars="$CR_ENV_VARS" \
    --update-secrets="OAUTH_CLIENT_ID=ge-demo-oauth-client-id:latest,OAUTH_CLIENT_SECRET=ge-demo-oauth-client-secret:latest,EXAMPLE_API_KEY=ge-demo-mcp-my-server-example-api-key:latest" \
    --region us-central1 \
...
EOF
  ...
  if [ "$APP_COUNT" = "1" ]; then
    ...
    REG_OUTPUT=$(python3 register_agent.py "$SELECTED_LOC" "$PROJECT_NUMBER" "$SELECTED_LOC" "$SELECTED_APP_ID" "$TOKEN" "ge-demo-mcp" "$SERVICE_URL/a2a/app" 'McpAgent' 'An MCP-enabled agent with GitHub and Workspace integrations.' "$AUTH_ID")
    echo "$REG_OUTPUT"
  ...
  echo "📂 Demo Asset Directory: ~/ge-demo-mcp"
  echo "📊 BigQuery Dataset:    ge_demo_mcp_ds"
  echo "🔥 Firestore:           ge-demo-mcp-data"
  
  echo ""
  echo "🔌 Custom MCP Servers (1)"
  echo "---------------------------------------------------------"
  echo "  #1: my-mcp-server (port 9090)"
  echo ""
  ...
  echo "   👉 https://console.cloud.google.com/bigquery?referrer=search&project=$PROJECT_ID&ws=!1m4!1m3!3m2!1s$PROJECT_ID!2sge_demo_mcp_ds"
  echo ""
  echo "========================================================="
  echo ""
  echo "💡 Next Steps:"
  ...
  echo "  $ cd ~ && bash setup-ge-demo-mcp.sh --cleanup"
  echo "========================================================="
  exit 0
