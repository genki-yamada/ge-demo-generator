...
export function makeGeRegistrar({ getToken, fetchImpl, config }) {
  ...
  return {
     ...
     */
    async setIngressAll(serviceName, region) {
      const serviceUrl =
     ...
     */
    async grantInvokerToDeSa(serviceName, region) {
      const baseUrl =
        `https://run.googleapis.com/v2/projects/${config.projectId}` +
        `/locations/${region}/services/${serviceName}`;
     ...
     */
    async registerAgent({ demoId, serviceUrl }) {
     ...
     */
    async registerToGe({ demoId, region }) {
      const resolvedRegion = region || config.agentRegion;

      // 1. Open ingress and obtain the service URI
      const { changed: ingressChanged, uri } = await this.setIngressAll(demoId, resolvedRegion);

      // 2. Grant Discovery Engine SA run.invoker on the service
      await this.grantInvokerToDeSa(demoId, resolvedRegion);

      // 3. Register the agent card in GE
      const r = await this.registerAgent({ demoId, serviceUrl: uri });
