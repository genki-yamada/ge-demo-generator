...
## Phase 2 E2E（フル機能: build→active→cleanup→deleted）で得た知見

新アーキ（Plan B〜D + 結線 + UI + cleanup）を `ge-work-osaka` に実デプロイし、`/api/generate`（実 Vertex
planning→実 ~600KB スクリプト生成→Firestore 登録→GCS 保存）と provisioner Cloud Run Job による実スクリプト
実行（build/`--cleanup`）を検証した際の知見。

### 検証できたこと（実 GCP）
- `terraform apply`（backend/provisioner イメージ、env、runtime/runner SA IAM、scripts バケット、provisioner Job）。
- アプリは ADC で実 Firestore 読み書き・実 Vertex planning（`gemini-3.5-flash`）→ `generateSetupScript` → GCS 保存。
- `deinteractivize` が実スクリプトで対話 `read -p` を完全除去（0 残存）。
- provisioner Job が実スクリプトを実行し **BigQuery データセット+テーブルを作成**。
- `--cleanup` 実行で **BQ データセット削除**（存在しないリソースは警告でトレラント、exit 0）= cleanup 機構の実証。

### 「build→active」完走に必要な環境前提（terraform/Dockerfile に反映済みのものと、運用者判断のもの）
1. **aiplatform.googleapis.com 有効化**（terraform `local.services` に追加済み。未有効だと planning が 403）。
2. **provisioner イメージのツール**: `jq make git python3 uv curl`（生成スクリプトの必須チェック。Dockerfile 反映済み）。
3. **provisioner Job メモリ 4Gi/2cpu**（512Mi だと並列 BQ ロードで OOM。terraform 反映済み）。
4. **runner SA に scripts バケット `roles/storage.objectViewer`**（Job entrypoint の `gsutil cp $SCRIPT_REF` 用。terraform 反映済み）。
5. **(default) Firestore データベースが必要**（生成デモの viewer は `(default)` DB に書き込む。本プロジェクトは Generator 用
   名前付き DB `generator` のみ＝`setup_fs.py` が `NOT_FOUND: database (default) does not exist` で失敗）。
   → デモをホストするプロジェクトに `(default)` Firestore DB を作成する必要がある（terraform 未管理。運用者が用意）。
6. **runner SA の IAM 自己付与**: 生成スクリプトは実行アイデンティティ（GAS では owner/Cloud Shell）から
   compute SA へ各種ロールを `gcloud projects add-iam-policy-binding` で自己付与する。scoped な runner SA では
   `projectIamAdmin` 権限が無く全付与が **警告**で失敗する（後続の viewer/関数が BQ/Firestore/Vertex 権限不足になりうる）。
   → 選択肢: (a) runner SA に `roles/resourcemanager.projectIamAdmin` を付与、(b) compute SA へ必要ロールを事前付与、
   (c) owner 相当のアイデンティティで実行。いずれも共有プロジェクトへの広範な権限付与のため運用者の明示判断が必要。
7. **Gemini Enterprise（discoveryengine）/ Vertex Agent Engine**: build 後半の GE 登録・Agent Engine 作成は
   GE Enterprise 製品アクセス（組織側）と Vertex quota に依存。環境固有のため別途確認。

> まとめ: コア移植（planning/codegen/registry/storage/deinteractivize/Job 実行/cleanup）は実 GCP で動作実証済み。
> 「フル active デモ」までの完走は上記 5〜7 の共有プロジェクト・プロビジョニング/権限/GE アクセスの整備が前提。

### Agent Engine オーファン問題の解消（AE-1〜AE-4）

Phase 2 E2E で Vertex Agent Engine（Sandbox）が削除されずオーファンとして残留する問題が確認された（1サイクルで
11件発生）。原因は、生成スクリプトが `AGENT_ENGINE_NAME` を `~/<dirName>/.env` に書き出すが、Build と Cleanup が
**別コンテナ**で実行されるため Cleanup 時にそのファイルが存在しないこと。

この問題は **ADR-0005**（entrypoint による GCS persist/restore）で対処済み。

- **Build 完了後**: Job entrypoint が `AGENT_ENGINE_NAME=` の1行だけを抽出し GCS（`gs://<bucket>/envs/<demoId>.env`）
  に保存する。
- **Cleanup 実行前**: entrypoint が GCS から復元して `.env` を再構築してからスクリプトを起動する。生成スクリプト
  本体は変更なく、Agent Engine を `force=true` で自動削除する。

**本変更以降に生成されたデモについては、Cleanup 後に AiPlatform REST API で手動削除する作業は不要。**

旧デモ（本変更前に生成）は引き続き entrypoint が `ENV_REF` を持たないため、手動削除が必要になる場合がある：

```bash
# オーファンエンジンの一覧
gcloud ai reasoning-engines list --location=us-central1 --project ge-work-osaka

# 個別削除
gcloud ai reasoning-engines delete <ENGINE_ID> --location=us-central1 --project ge-work-osaka
```
